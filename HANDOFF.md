...
### 0.0000000 2026-09-02：4 m 等边三角形布局的空场门控改用 trial 内判据

`baseline_check.py` 的「trial 间 <1%」判据**不适用于 4 m 等边三角形布局**。
该布局改用 trial 内判据，详见 `docs/experiments/20260902_gate_criterion_change.md`。

判据（仅限本布局）：
1. 每条空场 trial 的 trial 内漂移（前后半差异）< 2%
2. 人体事件段效应 / trial 内漂移 > 10×
3. trial 间差异只记录，不作放行条件

依据：09-01 的 eq4m 参照批次（全部 M1/M3 结论的来源）自己也过不了原判据，
且更差——入场前空场窗两两距离 node1 中位 2.601%、node3 中位 4.687%，
p90 达 11.5%；09-02 反而更稳（2.4% / 2.9%）。09-01 那批当日**未跑过门控**。
历史最好值 0.502% 出自 1 m 链路，链路短得多。

1 m / 3 m / 5 m 直线布局继续沿用原判据。

**附带结论**：M1/M3 各条件 12-13 个百分点的离散度远大于空场漂移的 2-5%，
因此来自姿势、站位精度、呼吸相位等真实物理变异，不是测量噪声。补重复次数
能收敛它；改善射频稳定性不能。

**操作教训**：`csi_lab.sh:38` 默认 `CSI_CHANNEL="2412 20"`（信道 1，
2.4 GHz 最拥挤），而 eq4m 全批在 2462。09-02 首次门控漏掉该环境变量，
跑在错误信道上并据此误判为「热瞬态」，浪费约 40 分钟。**起批必须显式带
`CSI_CHANNEL="2462 20"`**；两个 eq4m 批次文件头部已加警告。

### 0.000000 2026-08-28：原始数据已归档到项目私有 OneDrive，本地已清理

原始 trial 已按阶段归档到项目私有 OneDrive 的 `csi_lab_backup`。历史归档包括
`20260806_07_early`、`20260811_pilot`、`20260814_link_selectivity` 和
`20260820_link_integrity`；本次新增 `20260824_28_distance_diagnostics`，包含 9 项、
2.66 GB：三份 tar、SHA-256 清单、trial 目录和三份诊断说明。

远端逐项核对完成后，本机 60 个已归档 trial、旧位置 5 个重复副本、本地 8 月 20 日
归档及上传暂存目录已永久删除。Mac 可用空间由约 61 GiB 增至约 89 GiB。
`/Users/d-low/Desktop/Group Project/csi_results/` 仍保留索引、评估说明和布局照片，
后续采集继续写入该目录。需要历史原始数据时从私有 OneDrive 下载，并用相邻的
`SHA256SUMS` 或 `.sha256` 文件校验；公开 Git 仓库不保存 OneDrive 共享链接。

本次清理不改变实验判定：300 秒连续空场是高价值失败诊断，不进入距离准确率或
灵敏度统计；正式距离实验仍暂停，下一步仍是交换 node1/node3 物理位置后的 300 秒
空场判别。

### 0.00000 2026-08-28：1 m 连续空场仍严重漂移，正式距离实验继续暂停

为区分“跨 trial 射频重置”与“连续会话内漂移”，在node2载板恢复后执行了一条
300秒连续空场：监控接口、logger与injector全程不重启，2462 MHz、20 MHz，
共同匹配57,681包，node1/node3匹配率 `98.965% / 98.129%`，时间戳单调。

丢弃前30秒后，node1前后半频谱差异仍为 `2.527%`，node3达到 `12.884%`；
node3在60–150秒相对参考变化 `10.354%–13.614%`，180–210秒又恢复到`1.926%`。
RSSI中位数全程保持 `-36/-35 dBm`。变化覆盖node3两根天线和大多数子载波，
不是丢包、少量坏子载波、宽带AGC或单调热漂。

**结论：跨trial重新初始化不是主因。** 首选解释是node3路径附近的时变多径/物理变化；
仍需通过交换node1/node3物理位置排除接收机自身频率选择性状态跳变。当前不得开始
金属板或人体正式距离采集。完整报告：
`docs/experiments/20260828_continuous_drift_diagnosis.md`；复算脚本：
`scripts/continuous_drift.py`。原始951 MiB数据仍在Mac `csi_results/`，不进普通Git。

### 0.0000 2026-08-28：node2 载板故障（CMOS 电池没电），整机"有线无线同时断"

**症状极具迷惑性，排查方向一路错了三次，真因在载板。留此记录避免重蹈。**

现象：node2 既连不上管理网（三台机器对它的 ARP 全部 `INCOMPLETE`、线上抓包 0 个），
本机也连不上 Wi-Fi。最初判断为"天线断了"，错误。

**排查链（每一步排除一类原因）：**

| 测试 | 结果 | 排除了什么 |
|---|---|---|
| `ping6 ff02::1%en5` | 只有 Mac + node1 + node3 应答 | IPv4 配置错误。二层发现不依赖 IP，是最可靠的探测手段 |
| 从 node1/node3 反向 ping node2 | 均 `INCOMPLETE` | Mac 侧的 USB 网卡、网线、交换机端口 |
| 更换网线（两端） | 无变化 | 线缆 |
| 在 node1 上 `tcpdump` 30 秒 | **0 包** | "node2 的网络栈在运行"。绝对静默说明接口根本不存在 |
| `ip -br link` | 只有 `lo` | NetworkManager 配置问题 |
| `lspci` | **无任何网络设备**，只剩一个空的 PCI bridge | 驱动、内核、软件层全部无关 |

**决定性对照**：健康的 node1 有两个 PCIe 根端口
（`00:1c.0`=54bb → 总线 01 AX210；`00:1c.6`=54be → 总线 02 Realtek RTL8111）；
故障的 node2 只剩一个 `00:1c.0`(54be)，且后面是空的。
两块网卡都是 PCIe 设备，而 USB 设备（键鼠、AX210 蓝牙）全部正常——
**"USB 全好、PCIe 全没"就是这个故障的特征签名。**

**真因**：BIOS 里日期显示 **2023-01-01**，CMOS 已被重置 → 纽扣电池没电。
电池在**载板**上。

**故障件用交换法定位**：LattePanda Mu 是 260 针 SO-DIMM 计算模块，可与载板分离，
内存与 eMMC 都在模块上（见 §4.1）。旧模块 + 新载板 → 完全正常，
据此判定**载板故障、模块完好**，**零重装**恢复了全部环境：
内核 6.5.0-15、补丁版 iwlwifi（srcversion `1DEF5F9EE1363E0C6199DBF`）、
sudo 白名单、静态 IP、SSH 公钥、节点脚本（三方校验和一致）。

**AX210 MAC 未变，仍为 `e0:d5:5d:4d:01:e8`**，分析管线不受影响。
有线网卡 MAC 变为 `f2:37:90:08:94:b8`（新载板的 Realtek 芯片）。

**四条可复用的教训：**

1. **"有线和无线同时断"要优先怀疑共因，不要当成两个独立故障。**
   两块网卡都挂在 PCIe 上，一个 PCIe 问题就能同时打掉它们。
2. **`lspci` 应该排在 `nmcli` / `ip addr` 之前。** 先确认设备在不在总线上，
   再谈驱动和配置。本次在 NetworkManager 方向浪费了一轮。
3. **被动抓包是判断"主机是否在线"的可靠手段。** 一台启动完成的 Linux
   即使没有 IP 也会发 DAD、路由请求、mDNS；30 秒绝对静默 = 接口不存在。
4. **BIOS 里的日期是免费的 CMOS 电池检测。** 注意 systemd 会用磁盘上保存的
   时间戳把 OS 时钟往前顶，**OS 里看到的时间不是 RTC 真值，必须进 BIOS 看**。
   本次 OS 显示 8 月 27 日、BIOS 显示 2023 年 1 月 1 日，正是这个差异暴露了电池问题。

排查耗时约两小时，"天线坏了""网线坏了""NetworkManager 没跑"三个方向全错。
完整判据见 §10.9。
...
| 载板 | 计算模块经 **260 针 SO-DIMM 插槽**与载板连接，**可分离**。内存与 eMMC 都在模块上，因此系统随模块走；CMOS 纽扣电池在载板上。故障定位可用交换法，见 §10.9 |
...
**上表只记 AX210 的 MAC，有线网卡的 MAC 不作为标识。** 有线网卡是 Realtek RTL8111，
其 MAC 为本地管理地址、由驱动生成（三台分别观察到 `2a:`、`f2:`、`6e:` 开头），
**可能每次开机变化**，更换载板必然变化，不可用作长期标识或 Wake-on-LAN 目标。
node2 于 2026-08-28 换载板后为 `f2:37:90:08:94:b8`。

管理网的静态 IP 按 **`enp2s0` 接口名**绑定、**不绑 MAC**，因此换载板后自动生效，
无需改配置。另注意 node2 上该连接的 profile 名为「有线连接 1」而非 node1/node3 的
`mgmt`，配置内容一致（`ipv4.method manual` / `192.168.50.12/24` /
`ipv4.never-default yes`），仅名称不同。
...
### 10.9 有线与无线同时断：先查 `lspci`，再查 BIOS 电池

2026-08-28 在 node2 上实际发生一次，完整排查链见 §0.0000。

**症状**：管理网完全不通（三台机器对它的 ARP 均 `INCOMPLETE`），本机也连不上 Wi-Fi，
但键盘、鼠标、AX210 蓝牙等 USB 设备全部正常。

**第一步判据（按此顺序，最省时间）：**

```bash
lspci | grep -iE 'ethernet|network'   # 空 → 设备不在总线上，与驱动/NM/网线/天线全部无关
ip -br link                            # 只有 lo → 确认接口不存在
```

设备不在总线上时，**不要再查 NetworkManager、网线、天线**，它们都在更上层。
两块网卡（Realtek 有线 + AX210 无线）都是 PCIe 设备，一个 PCIe 故障即可同时打掉，
而 USB 设备不受影响。**"USB 全好、PCIe 全没"是这个故障的特征签名。**

健康节点应能看到两个 PCIe 根端口和两个设备，可拿另一台做对照：

```bash
lspci | grep -i 'pci bridge'          # 期望两个：00:1c.0 (54bb) 与 00:1c.6 (54be)
lspci | grep -iE 'ethernet|network'   # 期望 01:00.0 AX210 + 02:00.0 Realtek RTL8111
```

**第二步：看 BIOS 里的日期时间。** 若为 2023-01-01 之类的出厂值，即 CMOS 被重置，
纽扣电池没电（电池在载板上）。**必须进 BIOS 看**——systemd 会用磁盘上保存的时间戳
把 OS 时钟往前顶，OS 里显示的时间不是 RTC 真值。

**第三步：交换法定位故障件。** Mu 模块与载板可分离（§4.1）。把模块插到已知完好的
载板上：正常则载板故障、模块完好，可零重装恢复全部环境；仍异常则模块故障。
**注意必须换组合才有信息量**——在原载板上反复重启不会产生任何新证据。

**远程排查的两个可靠手段**（IPv4 配置坏掉时依然有效）：

```bash
ping6 -c 3 ff02::1%en5     # 链路上所有 IPv6 主机都会应答，不依赖 IPv4
# 在同网段的另一台节点上被动抓包，过滤掉所有已知 MAC：
sudo tcpdump -i enp2s0 -e -n 'not ether host <MAC1> and not ether host <MAC2>'
```

一台启动完成的 Linux 即使没有 IP，也会发 DAD、路由请求、mDNS。
**30 秒绝对静默 = 网络接口不存在**，而不是"IP 配错了"。
判断时务必先确认目标机确实已进入系统（接显示器），否则"没开机"与"没网卡"无法区分。
