# Person-in-WiFi-3D 实验项目交接文档（历史记录）

> **增强版说明：** 本文件保留原实现历史，其中部分章节仍描述旧的 HE-SU/234-tone 流程，或把 RGB-D 帧简称为“姿态真值”。增强版的部署方法、严格验收条件、HT/VHT 对照配置、主 CSI 要求和当前科学边界以 `README.md` 与 `CHANGELOG_IMPROVED.md` 为准。RGB-D 只属于参考测量，必须经过姿态估计后才会形成关节点标签。
...
### 0.00 2026-08-06 现场：采到 3 组有效空场，但发现 CSI 对人体几乎无响应（头号问题）

**当前最重要的未解问题。继续采数据前必须先解决，否则采到的都是无效样本。**
...
### 0.01 2026-08-06：H.265 彩色流修复 + 本次数据清单
...
`empty_05m` 三组可以保留进最终数据集；`walk_*` 全部需要在解决 §0.00 的问题后重采。
...
### 0.0 2026-08-06：开机后内核自动切到 6.8，已修复并锁定
...
CAM_HOST=node1 ./csi_lab.sh check           # 连相机一起预检

CAM_HOST=node1 ./csi_lab.sh empty_01 30     # 单次采集，带相机
./csi_lab.sh empty_01 30                    # 单次采集，不带相机

CAM_HOST=node1 ./csi_lab.sh batch phase_c.txt   # 批量采集
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `CAM_HOST` | 空 | 相机宿主。**留空则不采相机，没有姿态真值** |
...
```bash
cd ~/csi_lab
CAM_HOST=node1 ./csi_lab.sh batch phase_c.txt
```
...
- node3 视实验意图决定跟随或固定，但**必须记录**

每换一档距离都要重新记录布局，并重跑一遍三个场景：

```bash
CAM_HOST=node1 ./csi_lab.sh batch phase_c_05m.txt   # 摆到 5 m 后
CAM_HOST=node1 ./csi_lab.sh batch phase_c_10m.txt   # 摆到 10 m 后
CAM_HOST=node1 ./csi_lab.sh batch phase_c_15m.txt   # 摆到 15 m 后
```
...
2. **相机宿主节点磁盘。** 深度数据只有 14 GB 左右的余量（node1 剩 32 GB），
   27 组约 17 GB 的相机数据会很紧张。**每跑完一档就清理节点侧数据：**

   ```bash
   ssh node1 'rm -rf ~/csi_data/*'   # 确认 Mac 已回收后再执行
   ```
...
| 6 | **布局记录** | **仍未做**，节点物理位置未固定 |
| 7 | 空场数据在重复试验中有可解释 baseline | 未做，需正式采集 |
| 8 | 人穿过不同链路时对应 link 有可重复差异 | 未做，需正式采集 |
| 9 | 完成两接收端 packet-level 对齐并给出 coverage | **已达成**（95.2%） |
| 10 | 深度相机与 CSI 联合采集、时间可对齐 | **已达成**（2026-08-05） |
| 11 | 相机内参随数据保存，可还原三维坐标 | **已达成**（自动写入） |
| 12 | 批量采集可无人值守连续跑多组 | **已达成**（实测 3 组） |

剩余的 6/7/8 三项**仍然都卡在同一件事**：**先固定并记录节点布局，再做正式的重复采集**。
这是从 7-31 拖到现在的唯一实质性阻塞，且纯属现场工作，与软件无关。

仍未核实：**三台的物理天线实际根数**（数据里 `nRx=2`，与硬件是否相符存疑）。

Group 1 完整交付还需：

1. 深度图 ground truth 接入
2. CSI 与 depth 的同步与标定
3. 可复现的数据目录结构与 dataset schema
4. 至少一个端到端样例：从采集到对齐后的 CSI/depth pair
