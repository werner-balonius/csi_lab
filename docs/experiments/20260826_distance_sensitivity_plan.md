...
> **执行状态更新（2026-08-31）：3 m 人体批次已完成。** node1 通过边缘空场
> 门控后完成 3 偏移 × 2 状态 × 3 重复共 18 条，全部保留；CSI 频谱试验级
> 检出率 18/18，窗口级平衡准确率 83.9%–93.4%。node3 空场失败，只作定性
> 对照。3 m `plate_mid`、撤板空场和最后的 `plate_rx` 尚待完成；人体不需重采。
> 结果与 QA 见
> [`20260831_distance_offset_3m_human_batch.md`](20260831_distance_offset_3m_human_batch.md)。

> **执行状态更新（2026-08-31，同步另一台采集设备）：1 m 人体批次已完成。**
> 本机先完成 300 秒严格空场（node1/node3 `0.957% / 0.952%`）和中点金属板
> 闭环复测；撤板频谱残差 `5.18% / 7.00%` 未达到 `2–3%`，因此后续采用
> “先重新建立人体会话基线、人体批次完成后再做物理控制”的调整顺序。另一台设备
> 随后完成 3 偏移 × 2 状态 × 3 重复共 18 条正式人体 trial，全部有效。结果见
> [`20260830_distance_offset_1m_human_batch.md`](20260830_distance_offset_1m_human_batch.md)；
> 闭环诊断见
> [`20260830_plate_mid_clean_closure_retest.md`](20260830_plate_mid_clean_closure_retest.md)。
> Git 当前只有脚本与分析报告，另一台设备上的 18 条原始 CSI 尚未同步到本机。
